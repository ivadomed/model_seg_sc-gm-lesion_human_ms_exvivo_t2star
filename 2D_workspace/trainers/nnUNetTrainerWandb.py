...
from .visualization_2D import decode_bitmask_to_7_channels, plot_edge_map, plot_comparison_and_segmentation
...
class nnUNetTrainerWandb(nnUNetTrainer):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
        ...
        # --- Config-driven overrides (single source: $NNUNET_EXP_CONFIG json) ---
        # Defaults above reproduce the winning combination (soft_loss + spatial_aug_2). A config only
        # overrides what it names; it may set any exp_* flag and/or initial_lr.
        import json as _json, os as _os
        _cfgp = _os.environ.get("NNUNET_EXP_CONFIG")
        if _cfgp and _os.path.isfile(_cfgp):
            _cfg = _json.load(open(_cfgp))
            _cfg_exp = {k: v for k, v in _cfg.items() if k.startswith("exp_")}
            # If the config names ANY exp_* flag, it defines the COMPLETE active set:
            # reset all exp_* to False first, then apply (so defaults soft_loss+spatial_aug_2 don't leak).
            if _cfg_exp:
                for _a in [a for a in vars(self) if a.startswith("exp_")]:
                    setattr(self, _a, False)
                for _k, _v in _cfg_exp.items():
                    setattr(self, _k, _v)
            if "initial_lr" in _cfg:
                self.initial_lr = _cfg["initial_lr"]
            print(f"[nnUNetTrainerWandb] applied experiment config: {_cfgp} (active: "
                  f"{[a for a in vars(self) if a.startswith('exp_') and getattr(self, a)]})")
        ...
        # Historically most experiments used exactly ONE exp_* flag; the winning combination uses TWO
        # (soft_loss + spatial_aug_2). The downstream logic handles 1 or 2 (active_experiment_2 set only
        # when len==2). Allow either; reject 0 or >2.
        if len(active_experiments) not in (1, 2):
            raise ValueError(f"Expected 1 or 2 active exp_* flags, found {len(active_experiments)}: {active_experiments}")
    ...
    def on_train_start(self):
        ...
        # --- Config-driven epoch/lr override (also honors $NNUNET_NUM_EPOCHS for smoke tests) ---
        import json as _json, os as _os
        _cfgp = _os.environ.get("NNUNET_EXP_CONFIG")
        if _cfgp and _os.path.isfile(_cfgp):
            _cfg = _json.load(open(_cfgp))
            if "num_epochs" in _cfg: self.num_epochs = int(_cfg["num_epochs"])
            if "initial_lr" in _cfg: self.initial_lr = float(_cfg["initial_lr"])
        _ep = _os.environ.get("NNUNET_NUM_EPOCHS")
        if _ep:
            self.num_epochs = int(_ep)
            print(f"[nnUNetTrainerWandb] num_epochs overridden via env -> {self.num_epochs}")
        ...
        # config override: edge_params (soft-loss variants 2/3 differ only in these)
        if os.environ.get("NNUNET_EXP_CONFIG") and os.path.isfile(os.environ["NNUNET_EXP_CONFIG"]):
            import json as _jep
            _cep = _jep.load(open(os.environ["NNUNET_EXP_CONFIG"]))
            if "edge_params" in _cep:
                self.edge_params = {int(k): v for k, v in _cep["edge_params"].items()}
    ...
    def wandb_init(self):
        ...
        # Only attach the parameter-logging hook when wandb is actually active. Under
        # WANDB_MODE=disabled the run is a no-op and the watch hook crashes at validation.
        if getattr(wandb, "run", None) is not None and os.environ.get("WANDB_MODE", "").lower() not in ("disabled", "offline"):
