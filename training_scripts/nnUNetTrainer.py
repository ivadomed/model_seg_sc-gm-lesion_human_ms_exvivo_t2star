...
from nnunetv2.training.dataloading.nnunet_dataset import nnUNetDatasetNumpy, nnUNetDatasetBlosc2

class CachingLogicMixin(object):
    """
    This mixin contains the caching logic and can be combined with any nnU-Net dataset class.
    It's designed to be inherited alongside a dataset class.
    """
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._cache = {}
        # This message will now appear once for each worker process that gets created.
        print(f"RAM Caching enabled for process {os.getpid()}. This dataset instance has {len(self.identifiers)} cases.")

    def load_case(self, case_identifier):
        if case_identifier in self._cache:
            # Return a deepcopy to prevent augmentations from corrupting the cached version
            return deepcopy(self._cache[case_identifier])

        # If not in cache, load from disk using the parent class's method
        data, seg, properties, seg_from_prev_stage = super().load_case(case_identifier)

        # Ensure data is plain numpy array before caching. Check for None and dict types.
        if data is not None:
            data = data[:]
        if seg is not None:
            seg = seg[:]
        if seg_from_prev_stage is not None and not isinstance(seg_from_prev_stage, dict):
            seg_from_prev_stage = seg_from_prev_stage[:]

        self._cache[case_identifier] = (data, seg, properties, seg_from_prev_stage)
        return data, seg, properties, seg_from_prev_stage

# These new classes are pickleable because they are defined at the top level of the module
class CachingNNUNetDatasetNumpy(CachingLogicMixin, nnUNetDatasetNumpy):
    pass

class CachingNNUNetDatasetBlosc2(CachingLogicMixin, nnUNetDatasetBlosc2):
    pass


class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict,
                 device: torch.device = torch.device('cuda'), use_ram_caching:str = ''):
        ...
        print("[INVESTIGATION] nnunettrainer configuration :", configuration)
    ...
    def get_tr_and_val_datasets(self):
        ...
        tr_keys = 200 * tr_keys
        val_keys = 200 * val_keys
        if self.use_ram_caching:
            self.print_to_log_file("Using RAM caching. The dataset will be cached in RAM by worker processes.")

            # Determine which caching class to use based on the underlying dataset type
            if self.dataset_class is nnUNetDatasetNumpy:
                caching_class = CachingNNUNetDatasetNumpy
            elif self.dataset_class is nnUNetDatasetBlosc2:
                caching_class = CachingNNUNetDatasetBlosc2
            else:
                raise RuntimeError(f"RAM Caching is not supported for dataset type {self.dataset_class.__name__}")
            
            dataset_tr = caching_class(self.preprocessed_dataset_folder, tr_keys,
                                       folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage)
            dataset_val = caching_class(self.preprocessed_dataset_folder, val_keys,
                                        folder_with_segs_from_previous_stage=self.folder_with_segs_from_previous_stage)
        else:
    ...
    def get_dataloaders(self):
        ...
        print(f"Training dataset size: {len(dataset_tr.identifiers)} cases")
        print(f"Validation dataset size: {len(dataset_val.identifiers)} cases")
